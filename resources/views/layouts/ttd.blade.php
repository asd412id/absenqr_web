  ...
  <tr>
    <td style="width: 60%;white-space: nowrap">
      @if (strpos(Request::url(), route('absensi.log.index')) !== false)
        <table class=""
          style="margin-top: 25px;float: left;page-break-inside: avoid !important;white-space: nowrap;">
          <tr>
        ...
        <table style="width: 60%;margin-top: 25px;float: left;page-break-inside: avoid !important;white-space: nowrap;">
          ...
          <tr>
            <td class="status-color"
              style="display: inline-block;text-align: center;margin: 1px;border: solid 1px #000">
              &#10004;
            </td>
            <td style="padding-left: 10px">Hadir</td>
          </tr>
          <tr>
            <td class="status-color bg-warning"
              style="display: inline-block;text-align: center;margin: 1px;border: solid 1px #000"></td>
    ...
    <td style="text-align: right;white-space: nowrap !important">
      <table
        style="margin-top: 25px;float: right !important;page-break-inside: avoid !important;white-space: nowrap;text-align: left">
        <tr>
          <td style="height: 30px;">{{ @$config->kota ?? 'Sinjai' }},
            {{ \Carbon\Carbon::parse(@request()->end_date)->locale('id')->translatedFormat('d F Y') }}</td>
        </tr>
        <tr>
          <td>Mengetahui,</td>
        </tr>
        <tr>
          <td>Kepala {{ @$configs->kop ? 'Sekolah' : @$configs->nama_instansi ?? 'UPTD SMP Negeri 39 Sinjai' }}</td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="line-height: 1em;font-weight: bold">
            {!! @$configs->pimpinan
                ? nl2br($configs->pimpinan)
                : 'SITTI SAIDAH SUYUTI, S.Pd.,M.Pd.<br>
